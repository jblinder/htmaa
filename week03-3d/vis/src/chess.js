...
import peiceJSON from "./queen.json";
import recording from "./queen.mp3";
...
function init() {
  ...
  function buildVis(localData) {
    ...
    for (var i = 0; i < peiceJSON.length; i++) {
      ...
      const newCount = scale(play, [0.001, 0.4], [5, 12]);
      const geometry = new CylinderGeometry(newCount, newCount, 1, 50);
    ...
    const geometry = new TorusGeometry(10, 1, 40, 100);
    geometry.translate(0, peiceJSON.length / 100 + 1, 0);
    geometry.scale(0.5, 0.5, 0.5);
    const material = new MeshLambertMaterial({ color: 0xffff00 });
    const cube = new Mesh(geometry, material);
    cube.scale.set(0.5, 0.5, 0.5);
